...
impl Parser {
        ...
        while !self.is_at_end() && !self.check(Types::OrderBy) {
            ...
            match token.token_type {
                Types::Literal | Types::Identifier => {
                    self.parse_primary_condition(token)?;
                    continue;
                }
                Types::OpenParen => {
                ...
                Types::And | Types::Or => {
                    while let Some(token) = operator_parenthesis_stack.last() {
                        if token.token_type == Types::OpenParen
    ...
    fn parse_primary_condition(&mut self, token: Rc<Token>) -> Result<Condition, String> {
        ...
        match token.token_type {
            ...
            | Types::GreaterThanOrEqualTo => return Err("should not reach here".to_string()),
            _ => {
                let left = self.parse_expression_for_input(lhs)?;
